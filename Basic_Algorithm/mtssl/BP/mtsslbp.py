...
class mTSSLBP(torch.autograd.Function):
    @staticmethod
    def forward(ctx, inputs, network_config, layer_config,sg_p):
        ...
        for t in range(n_steps):
            mem_update = (-theta_m) * mem + inputs[..., t] #inputs是psp卷积后的结果
            mem += mem_update

            out = mem > threshold
            out = out.type(torch.float32)  #脉冲输出结果

            mems.append(mem)

            mem = mem * (1-out)+vrest*out   #复位
            outputs.append(out)
            # last_out=out
    ...
    def backward(ctx, grad_delta):  # grad_delta=L/a =L/u_j *u_j/a
        ...
        # print('grad_delta.shape:',grad_delta.shape)
        (delta_u, outputs, u, syns, others) = ctx.saved_tensors
        shape = grad_delta.shape
        # print('partial_L_partial_syns_j.shape:',grad_delta.shape)
        n_steps = shape[4]
        ...
        syn_a = glv.syn_a.repeat(shape[0], shape[1], shape[2], shape[3], 1)  # glv.syn_a.shape[1,1,1,1,20]
        partial_a = glv.syn_a / (-tau_s)
        partial_a = partial_a.repeat(shape[0], shape[1], shape[2], shape[3], 1) * outputs  #partial_a/t
        ...
        for t in range(n_steps - 1, -1, -1):
            ...
            out = outputs[..., t]
            partial_u = torch.clamp(-1 / delta_u, -8, 0)  # 将-1/delta_u[..., t]限制在[-8,0]
            partial_a_partial_u = partial_a * partial_u   #atp/utp
            #####下面是红色的部分
            for t_p in range(t, n_steps, 1):  # tp就是当m确定后 k的可能取值
                if t_p == t:
                    partial_uhtk_partial_uhtm = torch.ones_like(u[..., 0]).cuda()
                elif t_p == t + 1:

                    sg = (1 / sg_p) * torch.sign(torch.abs(u[..., t] - threshold) < (sg_p/ 2))
                    #sg = 1/(100*torch.abs(u[..., t] - threshold)+1.0)**2
                    partial_uhtk_partial_uhtm = (1 - theta_m)*(1-out) + (vrest - u[..., t + 1]) * sg
                else:
                    f1 = (1  - theta_m)*outputs[..., t :t_p-1]

                    sg = (1 /sg_p) * torch.sign(torch.abs(u[..., t:t_p - 1] - threshold) < (sg_p / 2))
                    #sg =  1/(100*torch.abs(u[..., t:t_p - 1] - threshold)+1.0)**2
            ...
            grad_tmp[syn < th] = grad_a[syn < th]  # 当syn<th成立时，grad_tmp对应位置的值替换成grad_a对应位置的值
            grad[..., t] = grad_tmp

        # print('grad_out.shape:',grad.shape)
