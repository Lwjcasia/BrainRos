...
from venv.network_parser import parse
from datasets import loadMNIST, loadCIFAR10, loadFashionMNIST, loadNMNIST_Spiking, loadImageNet
import logging
from utils.utils import learningStats, cnns, global_v as glv
...
def train(network, trainloader, opti, epoch, states, network_config, layers_config, err):
    ...
    for batch_idx, (inputs, labels) in enumerate(
            trainloader):  # inputs.shape[batchsize,channels,h,w]  label.shape=[batchsize]
        ...
        if network_config["rule"] == "TSSLBP":
            ...
            outputs = network.forward(inputs, epoch, True)  # true表示训练状态 返回的outputs是最后一层产生的psp
...
def test(network, testloader, epoch, states, network_config, layers_config, early_stopping):
    ...
    if test_accuracy > best_acc:
        ...
        df_cm = pd.DataFrame(cf, index=[str(ind * 25) for ind in range(n_class)],
                             columns=[str(ind * 25) for ind in range(n_class)])
        plt.figure()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-config', default='./configs/FashionMNIST_CNN.yaml', action='store', dest='config',
    ...
    logging.basicConfig(filename='result_fashionmnist_92.46.log', level=logging.INFO)
    ...
    elif params['Network']['dataset'] == "ImageNet":
        data_path = os.path.expanduser(params['Network']['data_path'])
        train_loader, test_loader = loadImageNet.get_imagenet(data_path, params['Network'])
    ...
    for e in range(params['Network']['epochs']):
        ...
        adjust_learning_rate(optimizer, e, start_lr=params['Network']['lr'])
