...
def train(epoch):
    ...
    for batch, (img, label) in enumerate(loaderbar):
        ...
        for i in range(batch_size):
            ...
            if data_num % 200 == 199:
                weight_updtknl = net.weight_update_kernel(A, B, max_dist_decay(max_distance, 2000, data_num),
                                                          sig_dist_decay(sig_dist, 2000, data_num))
...
if __name__ == "__main__":
    ...
    # 模型加载
    path = './results/CNN/model'
    model = CNN()
    model.load_state_dict(adjust_state_dict_keys(torch.load(path)))
    model.eval()
    # net = SOM_STDP()
    net = SOM_STDP()
    tau_LTP = 1
