...
def train(x_data, y_data, a_model, lr=1e-3, nb_epochs=10):
    ...
    for e in range(nb_epochs):
        ...
        for x_local, y_local in sparse_data_generator(x_data, y_data, batch_size, nb_T, nb_inputs):  # 生成器generator
            x_local = x_local.to_dense()
            for d in range(nb_T-nb_steps):
                x = x_local[:, d:(d+nb_steps), :]
                output, recs = a_model(x)
                _, spks = recs
                y_one_hot = torch.nn.functional.one_hot(y_local.long().to(output.device),
                                                        num_classes=output.size(2)).float()
                y = y_one_hot.unsqueeze(1).repeat(1, nb_steps + 1, 1)
                ...
                loss_val = loss_fn(output, y) + reg_loss
                optimizer.zero_grad()
                loss_val.backward()
                optimizer.step()
...
def test(x_data, y_data, a_model):
    a_model.eval()
    loss_fn = nn.MSELoss()
    with torch.no_grad():
        local_loss = []
        ...
        mean_loss = np.mean(local_loss)
        mean_acc = np.mean(acc_lst)
        print("test set: , loss=%.5f, acc=%.4f, time=%.2f" % (mean_loss, mean_acc, time.time() - time0))
        acc_lst = np.reshape(acc_lst, (-1, nb_T-nb_steps))
        timestep_acc = np.mean(acc_lst, axis=0)
    return timestep_acc


model = SNN_Model().to('cuda')
loss_hist, acc_hist = train(x_train, y_train, model, lr=1e-3, nb_epochs=50)  # 每个epoch得到的值
test_acc = test(x_test, y_test, model)  # 测试集只有一个epoch，这个是一个epoch中，每个时间步得到值
...
plt.figure(figsize=(10, 5))
plt.plot(range(1, len(test_acc) + 1), test_acc, marker='o', label='Accuracy')
plt.title('Accuracy at Each Time Step (Test set)')
