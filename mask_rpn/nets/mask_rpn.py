...
class Mask_RPN(nn.Module):
    def __init__(self, num_classes, use_figure_layers):
        ...
        if use_figure_layers:
            self.conv6 = layer.SeqToANNContainer(nn.Conv2d(1024, num_classes*self.figure_layers, kernel_size=1, padding=0))
        else:
            self.conv6 = layer.SeqToANNContainer(nn.Conv2d(1024, num_classes, kernel_size=1, padding=0))
