...
def fit_one_epoch(model, train_util, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, fp16, scaler, save_period, save_dir):
    ...
    with tqdm(total=epoch_step,desc=f'Epoch {epoch + 1}/{Epoch}',postfix=dict,mininterval=0.3) as pbar:
        for iteration, batch in enumerate(gen):
            ...
            rpn_loss, roi_loc, roi_cls, total = train_util.train_step(images, boxes, labels, 1, iteration, fp16, scaler)
            total_loss      += total.item()
            # rpn_loc_loss    += rpn_loc.item()
            # rpn_cls_loss    += rpn_cls.item()
            rpn_losses      += rpn_loss.item()
            ...
            pbar.set_postfix(**{'total_loss'    : total_loss / (iteration + 1), 
                                ...
                                'rpn_losses'       : rpn_losses / (iteration + 1), 
    ...
    with tqdm(total=epoch_step_val, desc=f'Epoch {epoch + 1}/{Epoch}',postfix=dict,mininterval=0.3) as pbar:
        for iteration, batch in enumerate(gen_val):
            ...
            with torch.no_grad():
                ...
                _, _, _, val_total = train_util.forward(images, boxes, labels, 1, batch)
