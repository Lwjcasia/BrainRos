...
class GModule(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.mode == 'reduce': 
            features = torch.mean(features, dim=2, keepdim=False) # t, b, h, w
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        self.stride = stride
        self.cnf = cnf
        # self.tam = TAModule(in_channels=planes, mid_channels=planes//4, time=time, mode='permute')
        self.LM = LModule(in_channels=planes, mid_channels=planes//8) 
        self.GM = GModule(time=time, mode='reduce') 
