...
SimpleLaserScan::SimpleLaserScan(ros::NodeHandle &n, int dir_num, std::string robot_name)
{
	...
	ROS_INFO("SIMPLE LASERSCAN NODE CREATED ...11111");
}
...
void SimpleLaserScan::scanCallback(const sensor_msgs::LaserScan::ConstPtr &msg)
{
	...
	// ROS_INFO("Size of ranges: %d", ranges_size);
	// // Only use the first half of ranges
    // int half_ranges_size = ranges_size / 2;
    // ranges.resize(half_ranges_size);  // Resize to keep only the first half
