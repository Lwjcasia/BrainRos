...
sys.path.append('../../')
from training.train_spiking_qsnn.qsnn_agent import AgentSpiking
from training.environment import GazeboEnvironment
from training.utility import *


def train_qsnn(run_name="SNN_R1", exp_name="Rand_R1", episode_num=(100, 200, 300, 400),
    ...
    """
    Training Spiking QSNN for Mapless Navigation
    ...
    env1_poly_list, _, _, _ = gen_rand_list_env1(episode_num[0])
    env2_poly_list, _, _, _ = gen_rand_list_env2(episode_num[1])
    env3_poly_list, _, _, _ = gen_rand_list_env3(episode_num[2])
    env4_poly_list, _, _, _ = gen_rand_list_env4(episode_num[3])
    ...
    while True:
        ...
        for ita in range(ita_per_episode):
            ...
            if len(agent.memory) > batch_size:
                ...
                tb_writer.add_scalar('Spike-QSNN/actor_loss', actor_loss_value, overall_steps)
                tb_writer.add_scalar('Spike-QSNN/critic_loss', critic_loss_value, overall_steps)
            ita_time_end = time.time()
            tb_writer.add_scalar('Spike-QSNN/ita_time', ita_time_end - ita_time_start, overall_steps)
            tb_writer.add_scalar('Spike-QSNN/action_epsilon', agent.epsilon, overall_steps)
            tb_writer.add_scalar('Spike-QSNN/raw_left_wheel_output', raw_snn_action[0], overall_steps)
            tb_writer.add_scalar('Spike-QSNN/raw_right_wheel_output', raw_snn_action[1], overall_steps)

            # Save Model
            if overall_steps % save_steps == 0:
                agent.save("../save_qsnn_weights", overall_steps // save_steps, run_name)
            ...
            if done or ita == ita_per_episode - 1:
                ...
                tb_writer.add_scalar('Spike-QSNN/avg_reward', episode_reward / (ita + 1), overall_steps)
                break
...
if __name__ == '__main__':
    ...
    if args.cuda == 0:
        USE_CUDA = False
    train_qsnn(use_cuda=USE_CUDA, batch_window=args.step)