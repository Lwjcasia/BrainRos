...
import pickle
sys.path.append('../../')
from evaluation.loihi_network.spiking_qsnn_net_loihi import SpikingQSNNNet
...
def evaluate_loihi_qsnn(pos_start=0, pos_end=49, model_name='qsnn_bw_5', save_dir='../saved_model/',
                         batch_window=5, is_save_result=False):
    """
    Evaluate Spiking QSNN in Simulated Environment on Intel's Loihi Neuromorphic Processor
    ...
    core_list = [core_list_input, core_list_hidden1, core_list_hidden2, core_list_hidden3, core_list_out,
                 core_list_bias]
    snn = SpikingQSNNNet(raw_weights, raw_bias, core_list, bias_amp=(1, 1, 1, 1))
...
if __name__ == '__main__':
    ...
    if args.save == 1:
        SAVE_RESULT = True
    MODEL_NAME = 'qsnn_bw_' + str(args.step)
    evaluate_loihi_qsnn(model_name=MODEL_NAME, is_save_result=SAVE_RESULT)