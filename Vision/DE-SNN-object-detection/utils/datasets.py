...
class LoadVOCImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, paths, img_size=416, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
                 cache_images=False, single_cls=False, pad=0.0):
        self.img_files = []
        self.label_files = []
        try:
            for path in paths:
                path = str(Path(path))  # os-agnostic
                parent = str(Path(path[0:path.index('ImageSets')])) + os.sep
                if os.path.isfile(path):  # file
                    with open(path, 'r') as f:
                        f = f.read().splitlines()
                        f_imgs = [parent + 'JPEGImages/' + x + '.jpg' for x in f]  # local to global path
                        f_labels = [parent + 'Annotations/' + x + '.xml' for x in f]  # local to global path
                elif os.path.isdir(path):  # folder
                    f = glob.iglob(path + os.sep + '*.*')
                else:
                    raise Exception('%s does not exist' % path)
                self.img_files.extend(f_imgs)
                self.label_files.extend(f_labels)
        except:
            raise Exception('Error loading data from %s. See %s' % (path, help_url))
        ...
        # Read image shapes (wh)
        # sp = path.replace('.txt', '') + '.shapes'  # shapefile path
        sp = str(Path(parent).parent) + '/{}'.format('train' if 'train' in path or 'val' in path else 'test') + '.shapes'
        try:
            with open(sp, 'r') as f:  # read existing shapefile
                s = [x.split() for x in f.read().splitlines()]
                assert len(s) == n, 'Shapefile out of sync'
        ...
        create_datasubset, extract_bounding_boxes, labels_loaded = False, False, False
        nm, nf, ne, ns, nd = 0, 0, 0, 0, 0  # number missing, found, empty, datasubset, duplicate
        np_labels_path = str(Path(parent).parent) + '/{}_labels.npy'.format('train' if 'train' in path or 'val' in path else 'test')  # saved labels in *.npy file
        if os.path.isfile(np_labels_path):
            s = np_labels_path  # print string
            x = np.load(np_labels_path, allow_pickle=True)
            if len(x) == n:
        ...
        for i, file in enumerate(pbar):
            if labels_loaded:
                ...
                try:
                    l = np.array(convert_annotation(file), dtype=np.float32)
                except:
                    nm += 1  # print('missing labels for image %s' % self.img_files[i])  # file missing
                    continue
