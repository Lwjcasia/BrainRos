...
import utils.quantification as qtf
...
def create_modules_Q(module_defs, img_size, cfg):
    # Constructs module list of layer blocks from module configuration in module_defs
    ...
    for i, mdef in enumerate(module_defs):
        ...
        if mdef['type'] == 'convolutional':
            ...
            stride = mdef['stride'] if 'stride' in mdef else (mdef['stride_y'], mdef['stride_x'])
            if isinstance(k, int):  # single-size conv
                modules.add_module('Conv2d', qtf.Conv2d_Quantification(in_channels=output_filters[-1],
                                                       out_channels=filters,
                                                       kernel_size=k,
                                                       stride=stride,
                                                       padding=k // 2 if mdef['pad'] else 0,
            ...
            if bn:
                modules.add_module('BatchNorm2d', qtf.BatchNorm2d_Quantification(filters, momentum=0.03, eps=1E-4,
                                                       timesteps=TIMESTEPS))
            else:
                routs.append(i)  # detection output (goes into yolo layer)
            ...
            pad = k // 2
            output_padding = (2 * pad - k) % stride  # output_padding
            modules.add_module('ConvTranspose2d', qtf.ConvTranspose2d_Quantification(in_channels=output_filters[-1],
                                                                     out_channels=filters,
                                                                     kernel_size=k,
                                                                     stride=stride,
                                                                     padding=pad if mdef['pad'] else 0,
...
class YOLOLayer_Q(nn.Module):
    ...
    def forward(self, p, out):
        p = qtf.Quantification.apply(p,TIMESTEPS)
        ASFF = False  # https://arxiv.org/abs/1911.09516
        if ASFF:
            i, n = self.index, self.nl  # index in layers, number of layers
            p = out[self.layers[i]]
...
class Darknet_Q(nn.Module):
    # YOLOv3 object detection model

    def __init__(self, cfg, img_size=(416, 416), verbose=False, timesteps=None):
        super(Darknet_Q, self).__init__()
        global TIMESTEPS
        TIMESTEPS = timesteps
        self.module_defs = parse_model_cfg(cfg)
        self.module_list, self.routs = create_modules_Q(self.module_defs, img_size, cfg)
        self.yolo_layers = get_yolo_layers(self)
        # torch_utils.initialize_weights(self)
