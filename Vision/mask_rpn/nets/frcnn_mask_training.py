...
def bbox_mapping(mask, bboxes, labels, S, T, figure_layers):
...
class ProposalTargetCreator(object):
    def __init__(self, n_sample=128, pos_ratio=0.5, pos_iou_thresh=0.3, neg_iou_thresh_high=0.3, neg_iou_thresh_low=0):
        self.n_sample = n_sample
        self.pos_ratio = pos_ratio
...
def ohem_loss(
    ...
    ohem_loc_loss = nn.SmoothL1Loss(loc_pred, loc_target, sigma=smooth_l1_sigma, reduce=False)
...
class FasterRCNNTrainer(nn.Module):
    ...
    def forward(self, imgs, bboxes, labels, scale, batch):
        ...
        mask_rpns, rois, box_counts = self.model_train(x = [base_feature, img_size], scale = scale, mode = 'rpn')
        ...
        for i in range(n):
            ...
            # mask作用为 为提取候选框用作标签
            mask_label = bbox_mapping(mask_label, bbox, label, S=8, T=T, figure_layers=5)  # 第一个4是步长，到这的图像大小应该是/8，第二个是时间步长，目前也设置为4
            # -------------------------------------------------- #
            ...
            # print(gt_roi_loc.shape)
            # print(gt_roi_loc)
            sample_rois.append(torch.Tensor(sample_roi).type_as(mask_rpns))

            sample_indexes.append(torch.ones(len(sample_roi)).type_as(mask_rpns) * roi_indice[0])

            gt_roi_locs.append(torch.Tensor(gt_roi_loc).type_as(mask_rpns))

            gt_roi_labels.append(torch.Tensor(gt_roi_label).type_as(mask_rpns).long())
